...
from glean_mcp.cookie_client import GleanClient
from glean_mcp.token_client import TokenBasedGleanClient
from glean_mcp.glean_filter import filter_glean_response
...
async def _test_query(client: GleanClient, query: str, page_size: int = 5, max_snippet_size: int = 200, save_results: bool = True, mode_suffix: str = ""):
    ...
    try:
        ...
        if save_results:
            ...
            filtered_filename = f"test_results/filtered_{safe_query}{suffix}_{timestamp}.json"
            os.makedirs("test_results", exist_ok=True)
            with open(filtered_filename, 'w') as f:
                json.dump(filtered_results, f, indent=2, ensure_ascii=False)

            # Save raw results for comparison
            raw_filename = f"test_results/raw_{safe_query}{suffix}_{timestamp}.json"
...
async def main():
    ...
    api_token = _sanitize(os.getenv("GLEAN_API_TOKEN"))
    cookies = _sanitize(os.getenv("GLEAN_COOKIES"))

    if not base_url:
        print("Error: GLEAN_BASE_URL environment variable is required")
        return
    if not api_token and not cookies:
        print("Error: provide either GLEAN_API_TOKEN or GLEAN_COOKIES in environment")
        return
    ...
    any_fail = False
    for mode_name, client in modes:
        try:
            print(f"\n{'='*60}\nMODE: {mode_name}\n{'='*60}")
            print(f"Testing {len(args.queries)} queries with Glean API ({mode_name})")
