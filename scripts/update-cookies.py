...
def restart_docker_container():
    ...
    try:
        ...
        if "glean-mcp-server" in result.stdout:
            ...
            subprocess.run(
                ["docker-compose", "restart", "glean-mcp-server"], check=True
            )
...
def test_connection():
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "scripts/test_and_filter.py", "test", "--no-save"],
            capture_output=True,
            text=True,
...
def main():
    ...
    parser.add_argument(
        "--no-restart", action="store_true", help="Don't restart Docker container"
    )
